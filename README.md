...
| `MAX_DRIFT` | `2.0` (config `max_position_drift`) | Stop walking further in a direction if a step's homed position drifts this many mm from the baseline's |
...
When a baseline exists, KAST doesn't blindly sweep the whole computed range either. It tests the baseline value first to get a trustworthy reference position, then steps one `SGT_STEP` at a time away from it, stopping in that direction the moment a step isn't 100% reliable or its homed position drifts more than `MAX_DRIFT` from the reference. This matters because StallGuard set too sensitive can trigger almost instantly rather than at the true endstop, Klipper still trusts that trigger and sets the logical position accordingly, and a macro backoff move applied after a false trigger becomes a real physical move with nothing to correct it. Repeated over several trials that compounds into the toolhead actually walking into the frame, this stops it at the first sign of that rather than after committing to a whole fixed range regardless. As a second line of defense, `macros/kast.cfg`'s `_KAST_HOME_AXIS` also verifies each individual homing move actually traveled a sane distance (`variable_min_home_travel`, default 5mm) before trusting it, and aborts the entire calibration immediately, before the backoff move, if not.
